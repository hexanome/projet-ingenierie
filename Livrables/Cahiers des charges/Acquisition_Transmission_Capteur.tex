...
\begin{itemize}
\item L'acquisition des données des capteurs �  un intervalle de temps définie préalablement,
\item L'envoi de ses données �  la base de la station �  un intervalle de temps régulier. Pendant cette connexion, la configuration des capteurs par la base devra être possible. Ainsi, l'écart entre chaque acquisition ou transmission devra être parametrable. Cette configuration sera faite pendant la transmission avec la base.
\end{itemize}
...
D'un point de vue plus technique, ce logiciel devra s'exécuter sur un micro-controleur attaché �  une puce ZigBee et au capteur.
...
Ce document va aussi porter sur le matériel exploité par le logiciel précédement cité, �  savoir le module de communication ZigBee. Les attentes par rapport �  celui-ci seront également décrite.

\subsection{Formulation des besoins, exploitation et ergonomie, expérience}

Le système présentement présenté doit répondre �  deux besoins bien distincts, et ce quelque soit la localisation géographique du système dans le monde :
...
\item Selon la configuration de la periode de transmission, soit la donnée du capteur est en enregistré temporairement, soit directement envoyé �  la base
...
\item Le module doit être capable de se reconnecter �  la base en cas de déconnexion
\item En cas de situation critique (batterie ou capteur), les données necessaire �  l'analyse de cette état devront être transmis �  la base le plus tôt possible
\end{itemize}

\paragraph{Transmission} Pendant la transmission, le système doit être capable d'analyser les données et d'envoyer les informations suivantes �  la base :
...
Pendant la transmission, le système doit être capable de recevoir des données correspondant �  la reconfiguration du module :
...
Le module d'acquisition doit sortir du mode "sommeil" �  un intervalle de temps régulier définie en configuration. Le système doit rester le moins de temps possible en mode "éveillé" pour respecter les contraintes d'autonomie.
...
L'acquisition de la batterie devra être effectué régulierement �  un intervalle de temps définie par la configuration. Une interface similaire �  celle du capteur sera utilisé pour reperer le niveau de batterie.
...
\item La periode de transmission, dans la plupart des cas cette periode sera égal �  la periode d'acquisition des données du capteur. 
\end{itemize}

\subsection{Contraintes d'utilisation}

Le système ne doit pas pouvoir être utilisé par un utilisateur humain. La sécurisation des données envoyées est à prévoir, il ne faut pas qu'un utilisateur externe puissent interagir avec le systeme. Cette interaction pourrait mener �  des disfonctionnements du système. Un système de cryptage, tel qu'un certificat, ou une autre méthode de sécurisation doit être mis en place.
